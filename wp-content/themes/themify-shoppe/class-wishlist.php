...
class Themify_Wishlist {
    ...
    public static function button($id = FALSE) {
		...
		<div class="wishlist-wrap">
			<a data-id="<?php echo $id ?>" onclick="javascript:void(0)" class="<?php echo!self::$is_wishlist_page ? 'wishlist-button' : 'wishlist-remove' ?><?php if (in_array($id, $wishlist)): ?> wishlisted<?php endif; ?>" href="#" rel="nofollow">
    ...
    public static function wishlist_page() {
        ...
        if( defined( 'ICL_LANGUAGE_CODE' ) ) {
                ...
                if($wpml_wishlist_page_id === self::$page_id){
                    self::$page_id = (int) self::get_wishlist_page(false);
                }
        }
        self::$is_wishlist_page = is_page() && self::$page_id == self::get_wishlist_page(false);
