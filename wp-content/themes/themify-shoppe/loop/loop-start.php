...
?>
<ul <?php echo  $width || $height?'data-width="'.$width.'" data-height="'.$height.'"':'';?> class="products <?php echo ( ( $themify->query_category != '' || is_archive() || ! empty( $themify->load_from_products_module ) ) && ! wc_get_loop_prop( 'is_shortcode' ) ) ? esc_attr( 'loops-wrapper ' . themify_theme_query_classes() ) : 'columns-' . esc_attr( wc_get_loop_prop( 'columns' ) ); ?>">
