...
function themify_theme_enqueue_scripts() {
	...
	if(true===Themify_Enqueue_Assets::$isInline){
		add_action('wp_head','themify_shoppe_font_css');
	}
	...
	if ( is_file( TEMPLATEPATH . '/custom_style.css' ) ) {
		themify_enque_style( 'custom-style', THEME_URI . '/custom_style.css' );
	}
	...
	if( themify_is_woocommerce_active() ) {
		...
		// Themify shop script
		wp_enqueue_script( 'theme-shop', themify_enque(THEME_URI . '/js/themify.shop.js'), array('jquery'), $theme_version, true );
		...
		// Variation scripts
		if($gallerType === 'default' && (is_shop() || is_product_category() || is_product_tag())){
			 wp_enqueue_script( 'wc-single-product' );
		}
	}
	
	if( $header != '' && $header !== 'default' && $header !== 'header-none'  && $header !== 'header-logo-center'  ) {
		themify_enque_style( $header, THEME_URI . '/styles/' . $header . '.css', $design_preset_dependency,$theme_version );
	}
	...
	$themify_script_vars = array(
		...
		'infiniteURL' => !themify_check( 'setting-infinite-url' ) ? 1 :'',
...
/**
 * Load Shoppe font when user select to output the CSS inline
 */
function themify_shoppe_font_css(){
    ob_start();?>
    <style>@font-face{font-family:'selimaregular';src:url(<?php echo THEME_URI; ?>/fonts/selima_-webfont.woff2) format('woff2'),url(<?php echo THEME_URI; ?>/fonts/selima_-webfont.woff) format('woff');font-weight:normal;font-style:normal}</style>
    <?php ob_end_flush();
}
...
function themify_theme_fixed_header() {
	...
	if(is_null($header_fixed)){
		...
		if ( in_array( $header, array( 'none', 'header-boxed-content', 'header-left-pane', 'header-right-pane', 'header-minbar-left', 'header-minbar-right' ),true ) ) {
...
if ( ! function_exists( 'themify_theme_body_class' ) ) {
	...
	function themify_theme_body_class( $classes ) {
		...
		if(themify_is_woocommerce_active()){
		    ...
		    if(!empty(WC()->cart) && sizeof(WC()->cart->get_cart())===0){
		...
		if ( ! in_array( $header, array( 'header-left-pane', 'header-right-pane', 'header-minbar', 'header-none' ),true ) ) {
			...
			if(!in_array($header,array('header-slide-out','boxed-content','header-slide-right','header-slide-left'))){
	...
	if( ! isset( $themify->builder_args['is_product'] ) || $is_product_query_page ) {
		...
		if (themify_is_query_page() || is_archive() || is_search() || is_tax() || is_home() || $is_product_query || $is_product_query_page){
			if( $is_product_query || is_tax('product_cat') ) {
				if( $themify->post_layout != 'auto_tiles' && !is_search() ){
...
function themify_theme_woocommerce_post_layouts( $arr ) {
	return array( 'list-post', 'grid2', 'grid3', 'grid4', 'list-thumb-image', 'grid2-thumb', 'auto_tiles' );
}
...
function themify_get_product_slider($product){
	static $hover_mode = null;
	global $themify;
	if(is_null($hover_mode)){
		$hover_mode = themify_theme_get('slider_hover','','setting-products_slider');
	}
	$is_slider_active = isset($themify->builder_args['slider_hover'])?$themify->builder_args['slider_hover']==='slider_hover':$hover_mode!=='disable';
	$images = array();
	if($is_slider_active){
		...
		if(!empty($images)){
			...
			$images = 'first_image' === $hover_mode ? $images[0] : implode(',',$images);
		}
	}
	return $images;
}
	...
	if($module->slug==='products' || $module->slug==='post'){
		...
		$append = array(
			...
			array(
				...
				'wrap_class' => 'tb_group_element_grid4 tb_group_element_grid3 tb_group_element_grid2 tb_group_element_grid2-thumb'
			),
			array(
				'id' => 'post_gutter',
				'type' => 'select',
...
function themify_theme_module_render_vars( $mod ) {
	global $themify;
	...
	if( isset( $mod['mod_name'],$mod['mod_settings']) ) {
		...
		if( $mod_name==='products' || $mod_name==='post') {
			...
			if( $is_product ) {
				...
				$content_layout = $content_layout === 'default'? themify_get( 'setting-product_content_layout' ): ($content_layout !== 'no' ? $content_layout : '');

				$post_gutter = $post_gutter === 'default'? themify_get( 'setting-post_gutter' ) === 'no-gutter': $post_gutter === 'no';

				$product_hover = ! empty( $mod_settings['disable_product_slider'] )? $mod_settings['disable_product_slider'] : '';
				...
				if ( $infinite = themify_theme_get( 'more_posts', 'infinite' ) ) {
					$themify->builder_args['infinite'] = $infinite;
				}
			} else {
				$masonary = $masonary === 'default'?! themify_check( 'setting-shop_masonry_disabled' ) : $masonary === 'yes';

				$content_layout = $content_layout === 'default'? themify_get( 'setting-post_content_layout' ) :( $content_layout !== 'no' ? $content_layout : '');
			...
			$filter_name = $is_product ? 'themify_theme_loops_wrapper_class': (! empty( $themify->builder_args ) ? 'themify_builder_module_loops_wrapper' : '');
...
add_filter( 'themify_builder_module_render_vars', 'themify_theme_module_render_vars' );
...
/**
 * Return the status of Rating stars.
 *
 * @return bool
 */
function themify_is_archive_rating_stars_disabled() {
    return (bool) themify_get( 'setting-hide_product_rating_stars' );
}
