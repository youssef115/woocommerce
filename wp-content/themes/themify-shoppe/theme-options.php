...
class Themify {
	...
	function template_redirect() {
		...
		if( is_page() ) {
			...
			if ( !in_array($postType, $excluded_types) ) {
				// set layout
				$sidebar_mode = array('sidebar-none', 'sidebar1', 'sidebar1 sidebar-left');
		...
		if(!is_search()) {
			...
			if($this->post_layout!=='list-large-image')
				$this->post_layout_type = themify_get( 'setting-search-result_content_layout', 'default');
			else
				$this->post_layout_type = 'default';
		}
	...
	private function themify_get_sticky_slider_value(){
		...
		if ( is_page() ) {
			...
			$option = 'setting-shop_archive_sticky_sidebar';
