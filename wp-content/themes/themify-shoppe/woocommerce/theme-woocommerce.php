...
function themify_get_product_image($urlonly=false,$src=false,$is_singular=false) {
	...
	$load_from_module = !$is_singular && is_singular('product') && !empty($themify->load_from_products_module);
	...
	if($is_singular && !$related && !$up_sells){
		...
		if(!empty($slider)){
			$hover_mode = themify_theme_get('slider_hover','','setting-products_slider');
			if('first_image' === $hover_mode){
				$second_image = $slider;
				$cl = $slider = '';
			}else{
				$cl = ' product-slider';
		...
		if(empty($second_image) && true === $themify->load_from_products_module && true === $themify->products_hover_image){
	...
	if(!themify_is_image_script_disabled()){
		...
		if($width || $height){
			...
			if(!empty($second_image)){
				$second_image = themify_do_img($second_image, $width, $height);
				$second_image = '<img src="'.esc_url($second_image['url']).'" class="themify_product_second_image" />';
			}
		}
	}
	if(empty($post_image)){
		if($urlonly){
			...
			if(!empty($second_image)){
				$second_image = wp_get_attachment_image_src( $second_image,$size );
				$second_image = '<img src="'.$second_image[0].'" class="themify_product_second_image" />';
			}
		}
